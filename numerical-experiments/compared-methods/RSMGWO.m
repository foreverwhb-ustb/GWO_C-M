...
while l<M
    ...
    for i=1:Num%采用EPD
        ...
        if isempty(temp)
            ...
            if rand<0.5
                Positions(N-Num+i,1:d)=temp+(ub-temp).*rand(1,d)*a;
            else
                Positions(N-Num+i,1:d)=temp-(temp-lb).*rand(1,d)*a;
            end
        end
    end
    ...
    Positions=BoundrayCheck(Positions,bb,aa,d);%狼只越界调整
