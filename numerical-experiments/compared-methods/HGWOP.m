...
valuess=zeros(1,iter);
g_best_t=zeros(1,D);
for t=1:iter    
    cr=0.5*(sin(2*pi*0.25*t+pi)*t/iter+1);      
    ax=2-t*((2)/iter);w=ax/2;
    ...
    for j=1:swarm_size        
        if  ((t>5) && (f_val(j)<=g_best_val_t(t-5)))||(j==1)
    ...
    for j=1:swarm_size
        if f_val(j)<p_best_val(1,j)
            p_best(j,:)=particles_x(j,:);
            p_best_val(1,j)=f_val(j);
        end
        if p_best_val(1,j)<g_best_val
            g_best=particles_x(j,:);
            g_best_val=p_best_val(1,j);
        end
        if f_val(j)<Alpha_score
            Alpha_score=f_val(j); % Update alpha
            Alpha_pos=particles_x(j,:);
    ...
    g_best_t(t,:)=g_best;
    g_best_val_t(t)=g_best_val;   
    % valuess((i-1)*swarm_size+1:i*swarm_size)=g_best_val;
    valuess(1,t)=g_best_val;
end
% valuess(1,:)=valuess(1:swarm_size:iter*swarm_size);